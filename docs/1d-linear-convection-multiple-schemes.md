...
- 2BD explicit preserves the shape of the triangle much better than BD 1st, exhibiting less
    numerical diffusion. However, it requires sigma <= 0.1, which is a value 10 times more
...
- Lax-Wendroff method preserves the wave shape mush better than 1st order schemes, with low
    numerical diffusion. Small oscilltions appear near sharp gradients due to dispersive truncation
    error. Works very well for smooth solutions but shows artifacts for non-smooth profiles.
## Key Tradeoff
...
- Higher accuracy (2nd order) with a relatively mild stability condition sigma <= 1. No need to
    solve linear systems, so it is computationally efficient per time step. Accuracy comes
    at a cost of dispersive oscillations near steep gradients.

## Problems
- The 2BD explicit scheme fails when sigma > 0.1 - much stricter conditions than one might expect
    based on CFL alone.
...
- Lax-Wendroff method generates non-physical oscillations (Gibbs-like) near discontinuities. Not TVD,
    so new extrema and overshoots may apear. Solution quality degrades for sharp initial conditions
    despite stability.